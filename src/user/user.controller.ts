import { Body, Controller, Get, Put } from '@nestjs/common';  //Entry 포인트 역할을 하는 Controller와 Get/Put 을 사용하기 위해 import 시켜줌
...
export class UserController {
  ...
  async findAll(): Promise<User[]> { //asnyc(비동기 작업)로 사용하는 정보로는 User 배열이 있음, Promise 사용
    const result = await this.userService.findAll(); // 반환되는 result는 userService를 호출하여 그곳에서 작업을 해주어 보내줌 (예정)
